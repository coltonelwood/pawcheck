...
export default async function TrainingPage() {
  ...
  const { data: plans } = await supabase
    .from('training_plans')
    .select('id, plan_title, plan_summary, status, progress_pct, duration_weeks, created_at, pet:pets(name, species)')
    .eq('user_id', user.id)
    ...
    <div className="container max-w-4xl py-8 lg:py-12">
      <div className="flex items-end justify-between mb-8 gap-4">
        <div>
          ...
          <p className="mt-2 text-ink-mute">
            AI-generated, positive-reinforcement plans for your pet.
          </p>
        </div>
      ...
      {!plans || plans.length === 0 ? (
        <div className="bg-card rounded-2xl border border-dashed border-cream-300 p-12 text-center">
          <GraduationCap className="w-12 h-12 mx-auto text-ink-mute mb-3" strokeWidth={1.5} />
          <p className="text-ink-mute mb-4">No training plans yet</p>
          <Button asChild variant="default">
            <Link href="/training/new">Create your first plan</Link>
          </Button>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-ink-soft line-clamp-1 mb-2">
                    {plan.plan_summary}
                  </p>
